...
echo "[2/8] Installing Node.js (v20 LTS)..."
...
echo "[4/8] Installing PostgreSQL and PostGIS..."
sudo apt-get install -y postgresql postgresql-contrib postgis postgresql-14-postgis-3
sudo systemctl enable postgresql
sudo systemctl start postgresql

echo "[4.5/8] Securing DB and setting up schema (Waiting 10 seconds for DB init)..."
sleep 10
...
echo "[5/8] Installing PM2 and build tools globally..."
...
echo "[6/8] Installing dependencies for Backend and TCP Server..."
...
# 7. Build Frontend
echo "[7/8] Installing and Building Frontend (React/Vite)..."
cd frontend
npm install --fetch-timeout=600000
npm run build
cd ..
...
echo "[8/8] Installing Nginx and mapping Daemons..."
...
pm2 start tcp-server/src/server.js --name "geosurepath-tcp"
